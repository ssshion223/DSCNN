...
module linebuf_bank_ram #(
    parameter DATA_W = 8,                          // 存储数据位宽
    parameter DEPTH  = 64,                         // RAM 深度
    parameter ADDR_W = (DEPTH <= 1) ? 1 : $clog2(DEPTH) // 地址位宽
)(
    input  wire                     clk,   // 时钟信号
    input  wire                     we,    // 写使能
    input  wire [ADDR_W-1:0]        addr,  // 读写地址
    input  wire [DATA_W-1:0] din,   // 写入数据
    output wire signed [DATA_W-1:0] dout   // 当前地址读出数据
);
    (* ram_style = "distributed" *)
    wire signed [DATA_W-1:0] din_s = $signed(din);
