...
module mfcc10_fft512_rtl #(
    ...
    wire [ADDR_W-1:0] tw_idx_full = butterfly_j << (NFFT - stage);
    wire [ADDR_W-2:0] tw_idx = tw_idx_full[ADDR_W-2:0];
    wire [ADDR_W:0] next_block = {1'b0, block_base} + step_size;

    wire signed [FFT_IN_W-1:0] in_re_q = in_re >>> (IN_W - FFT_IN_W);
    wire signed [FFT_IN_W-1:0] in_im_q = in_im >>> (IN_W - FFT_IN_W);
    wire signed [FFT_W-1:0] in_re_ext = {{(FFT_W-FFT_IN_W){in_re_q[FFT_IN_W-1]}}, in_re_q};
    wire signed [FFT_W-1:0] in_im_ext = {{(FFT_W-FFT_IN_W){in_im_q[FFT_IN_W-1]}}, in_im_q};
    ...
    wire signed [TW_W-1:0] tw_re;
    wire signed [TW_W-1:0] tw_im;

    assign in_ready = (state == S_LOAD);

    mfcc10_fft512_twiddle_rom u_twiddle_rom (
        .addr  (tw_idx),
        .tw_re (tw_re),
        .tw_im (tw_im)
    );
    ...
    function [ADDR_W-1:0] bit_reverse;
        ...
        integer bit_idx;
        begin
            for (bit_idx = 0; bit_idx < ADDR_W; bit_idx = bit_idx + 1) begin
                bit_reverse[bit_idx] = value[ADDR_W-1-bit_idx];
            end
        end
    endfunction

    function [MEM_W-1:0] pack_complex;
        input signed [FFT_W-1:0] re;
        input signed [FFT_W-1:0] im;
        begin
            pack_complex = {re, im};
        end
    endfunction
    ...
    always @(*) begin
        ...
        case (state)
            S_LOAD: begin
                ...
                ram_a_din  = pack_complex(in_re_ext, in_im_ext);
                ram_a_we   = load_fire;
            end
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            case (state)
                ...
                S_CALC_LATCH: begin
                    u_re_r <= $signed(ram_a_dout[MEM_W-1:FFT_W]);
                    u_im_r <= $signed(ram_a_dout[FFT_W-1:0]);
                    b_re_r <= $signed(ram_b_dout[MEM_W-1:FFT_W]);
                    b_im_r <= $signed(ram_b_dout[FFT_W-1:0]);
                    w_re_r <= tw_re;
                    w_im_r <= tw_im;
                    state  <= S_CALC_MUL;
                end
                ...
                S_OUTPUT_EMIT: begin
                    ...
                    out_re    <= $signed(ram_a_dout[MEM_W-1:FFT_W]);
                    out_im    <= $signed(ram_a_dout[FFT_W-1:0]);
